...
description: >-
  ...
  per-page conventions these sites follow. Complements each repo's AGENTS.md; defer to it
...
5. **Fail closed.** On a missing token, unknown id, or unauthorized user, stop
   ...
   confirm flag server-side (the house pattern is `!empty($_POST['confirm'])`).
...
- Remember what a green run does **not** cover here: the harness drives real HTTP but
  ...
  inside an installed PWA are checked by hand. That is where most real bugs in this stack
